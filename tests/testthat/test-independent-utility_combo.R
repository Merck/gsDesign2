...
for (i in 1:4) {
  ...
  delta_test1 <- gsDesign2:::gs_delta_wlr(arm0 = arm$arm0, arm1 = arm$arm1,
...
sigma2 <- gsDesign2:::gs_sigma2_combo(arm0 = arm$arm0, arm1 = arm$arm1, tmax = 30,
...
for (i in 1:4) {
  for (j in 1:4) {
    ...
    sigma_ij <- gsDesign2:::gs_sigma2_wlr(arm0 = arm$arm0, arm1 = arm$arm1,
...
theta <- rep(0, n_test)
analysis <- 1
prob <- gsDesign2:::gs_prob_combo(lower_bound = rep(lower, n_test),
...
p_efficacy <- gsDesign2:::pmvnorm_combo(lower = rep(upper, n_test),
                            upper = rep(Inf, n_test),
                            group = analysis,
                            mean = theta,
                            corr = corr)
p_futility <- gsDesign2:::pmvnorm_combo(lower = rep(-Inf, n_test),
...
c <- c(1,3)
corr1 = corr[c,c]
p_efficacy_1 <- gsDesign2:::pmvnorm_combo(lower = rep(upper[1], 2),
                              upper = rep(Inf, 2),
                              group = 1,
                              mean = theta[c],
                              corr = corr1)
p_futility_1 <- gsDesign2:::pmvnorm_combo(lower = rep(-Inf, 2),
                              upper = rep(lower[1], 2),
                              group = 1,
                              mean = theta[c],
                              corr = corr1)
p_efficacy_2 <- gsDesign2:::pmvnorm_combo(lower=c(lower[1], upper[2]),
                              upper=c(upper[1], Inf),
                              group = analysis,
                              mean = theta,
                              corr = corr)
p_futility_2 <- gsDesign2:::pmvnorm_combo(lower=c(lower[1], -Inf),
...
p <- gsDesign2:::pmvnorm_combo(lower = rep(lower, n_test),
