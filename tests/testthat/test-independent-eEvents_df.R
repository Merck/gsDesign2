test_that("expected events is different from gsDesign::eEvents and eEvents_df",{
  enroll_rate <- tibble::tibble(duration=c(2,1,2),rate=c(5,10,20))
  fail_rate <- tibble::tibble(duration=c(1,1,1),fail_rate=c(.05,.02,.01),dropout_rate=.01)
  total_duration <- 20
  testthat::expect_equal(eEvents_df(enroll_rate,fail_rate,total_duration,simple=TRUE),
                         eEvents(lambda=fail_rate$fail_rate,S=fail_rate$duration[1:(nrow(fail_rate)-1)],
                                 eta=fail_rate$dropout_rate,gamma=enroll_rate$rate,
                                 R=enroll_rate$duration,T=total_duration)$d,
                                 ignore_attr = TRUE)
})

test_that("data frame returned from eEvents_df not as expected",{
  # test case from gsSurvNPH
  enroll_rate <-tibble::tibble(duration=c(1,1,8),rate=c(3,2,0))
  fail_rate <- tibble::tibble(duration=c(4,Inf),fail_rate=c(.03,.06),dropout_rate=c(.001,.002))
  total_duration <- 7
  xx <- eEvents_df(enroll_rate,fail_rate,total_duration,simple=FALSE) %>% data.frame()
...
enroll_rate = tibble::tibble(duration = c(50),
                             rate = c(10))
fail_rate = tibble::tibble(duration = c(10, 20, 10),
                           fail_rate = log(2) / c(5, 10, 5),
                           dropout_rate = c(0.1, 0.2, 0))
total_duration = 5
simple = TRUE

###test1:with mutiple fail_rate, short FU
testthat::test_that(
  "expected events is different from double-programmed vs eEvents_df, with mutiple fail_rate, short FU",{
    testthat::expect_equal(
      test_Event(enroll_rate, fail_rate, total_duration),
      eEvents_df(enroll_rate, fail_rate, total_duration, simple)
    )
  })

###test2:with mutiple fail_rate, long FU
testthat::test_that("expected events is different from double-programmed vs eEvents_df,
                    with mutiple fail_rate, long FU",{
                      total_duration=80
                      testthat::expect_equal(test_Event(enroll_rate, fail_rate, total_duration),
                                             eEvents_df(enroll_rate, fail_rate, total_duration, simple))
                    })

###test3:with mutiple fail_rate and with mutiple enrollment duration
testthat::test_that("expected events is different from double-programmed vs eEvents_df,
                    with mutiple enrollment duration",{
                      enroll_rate=tibble::tibble(duration=c(50,10),
                                                 rate=c(10,5))
                      fail_rate=tibble::tibble(duration=c(10,20,10),
                                               fail_rate=log(2)/c(5,10,5),
                                               dropout_rate=c(0.1,0.2,0))
