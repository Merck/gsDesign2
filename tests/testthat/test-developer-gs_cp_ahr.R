...
test_that("Compare the gs_cp_ahr with gsDesign", {
  ...
  #  conditional power by gs_cp_ahr#
  # ------------------------------ #
  cp_gsd2 <- gs_cp_ahr(x = xu, i = 1, z_i = -qnorm(0.04))

  # ------------------------------ #
  #  conditional power by gsDesign #
  # ------------------------------ #
  cp_gsd <- gsCP(x = x_gsd, i = 1, zi = -qnorm(0.04))

  # ------------------------------ #
  #       comparison               #
  # ------------------------------ #
  # theta = H0
  # given IA1 observed blinded data and compute IA2 & FA conditional power
  expect_equal(cp_gsd$upper$prob[, 2] %>% cumsum(),
               cp_gsd2$upper[1, 2:3] %>% unlist() %>% as.vector(),
               tolerance = 5e-2)

  # under H1 assumed theta
  # given IA1 observed blinded data and compute IA2 & FA conditional power
  expect_equal(cp_gsd$upper$prob[, 3] %>% cumsum(),
               cp_gsd2$upper[2, 2:3] %>% unlist() %>% as.vector(),
               tolerance = 5e-2)
