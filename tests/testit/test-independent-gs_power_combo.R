assert("calculate analysis number as planned", {
  ...
  (isTRUE(all.equal(max(fh_test$analysis), max(gs_power_combo_test1$analysis$analysis))))

  # calculate analysisTimes as planned
  fh_test <- res$fh_test
  gs_power_combo_test1 <- res$gs_power_combo_test1

  (isTRUE(all.equal(unique(fh_test$analysis_time), unique(gs_power_combo_test1$analysis$time))))
  ...
  for (i in 1:max(fh_test$analysis)) {
    ...
    (isTRUE(all.equal(event, unique(gs_power_combo_test1$analysis$event)[i], tolerance = 0.01)))
  }
  ...
  (isTRUE(all.equal(max(fh_test$analysis), max(gs_power_combo_test2$analysis$analysis))))

  # calculate analysisTimes as planned
  fh_test <- res$fh_test
  gs_power_combo_test2 <- res$gs_power_combo_test2

  (isTRUE(all.equal(unique(fh_test$analysis_time), unique(gs_power_combo_test2$analysis$time))))
  ...
  for (i in 1:max(fh_test$analysis)) {
    ...
    (isTRUE(all.equal(event, unique(gs_power_combo_test2$analysis$event)[i], tolerance = 0.01)))
  }
})
