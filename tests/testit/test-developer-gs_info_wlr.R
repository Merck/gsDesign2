assert("statistcial information of WLR under logrank test is approximately same as that from AHR", {
  ...
  (isTRUE(all.equal(x1$info0, x2$info0, tolerance = 1e-2)))
  # info0 of WLR is approximately proportional to events
  (isTRUE(all.equal(x2$event / x2$info0, c(4, 4, 4), tolerance = 1e-2)))
})
