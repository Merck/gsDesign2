assert("Call with defaults", {
  x1 <- gs_design_ahr()
  x2 <- gs_design_ahr_()
  (isTRUE(all.equal(x1$analysis$time, x2$bounds$Time[x2$bounds$Bound == "Upper"], check.attributes = FALSE)))
  (isTRUE(all.equal(x1$analysis$event, x2$bounds$Events[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$bound$z[x1$bound$bound == "upper"], x2$bounds$Z[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$bound$probability[x1$bound$bound == "upper"], x2$bounds$Probability[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$analysis$ahr, x2$bounds$AHR[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$analysis$theta, x2$bounds$theta[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$analysis$info, x2$bounds$info[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$analysis$info0, x2$bounds$info0[x2$bounds$Bound == "Upper"])))
})

assert("Single analysis", {
  x1 <- gs_design_ahr(analysis_time = 40)
  x2 <- gs_design_ahr_(analysisTimes = 40)
  (isTRUE(all.equal(x1$analysis$time, x2$bounds$Time[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$analysis$event, x2$bounds$Events[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$bound$z[x1$bound$bound == "upper"], x2$bounds$Z[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$bound$probability[x1$bound$bound == "upper"], x2$bounds$Probability[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$analysis$ahr, x2$bounds$AHR[x2$bounds$Bound == "Upper"])))
...
assert("Multiple analysisTimes", {
  ...
  (isTRUE(all.equal(x1$analysis$info, x2$bounds$info[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$analysis$info0, x2$bounds$info0[x2$bounds$Bound == "Upper"])))
  (isTRUE(all.equal(x1$bound$z[x1$bound$bound == "lower"], x2$bounds$Z[x2$bounds$Bound == "Lower"])))
  (isTRUE(all.equal(x1$bound$probability[x1$bound$bound == "lower"], x2$bounds$Probability[x2$bounds$Bound == "Lower"])))
})
...
assert("Pre-specificed spending time", {
  ...
  (isTRUE(all.equal(x$bound$spending_time, c(12, 24, 36) / 36)))
  ...
  (isTRUE(all.equal((x$bound |> filter(bound == "upper"))$spending_time, c(12, 24, 36) / 36)))
  (isTRUE(all.equal((x$bound |> filter(bound == "lower"))$spending_time, c(15, 24, 36) / 36)))
})
...
assert("Spending time when the analyses are driven by information fraction", {
  ...
  (isTRUE(all.equal(x$bound$spending_time, 1:3/3)))
  ...
  (isTRUE(all.equal((x$bound |> filter(bound == "upper"))$spending_time, 1:3/3)))
  (isTRUE(all.equal((x$bound |> filter(bound == "lower"))$spending_time, x$analysis$info / max(x$analysis$info))))
  ...
  (isTRUE(all.equal((x$bound |> filter(bound == "upper"))$spending_time, 1:3/3)))
  (isTRUE(all.equal((x$bound |> filter(bound == "lower"))$spending_time, x$analysis$info0 / max(x$analysis$info0))))
})
...
assert("Spending time when some analyses are skipped", {
  ...
  (isTRUE(all.equal((x$bound |> filter(bound == "upper"))$spending_time, 1:3/3)))
  (isTRUE(all.equal((x$bound |> filter(bound == "lower"))$spending_time, x$analysis$info[2:3] / max(x$analysis$info))))
})
