...
check_fixed_design_output <- function(result) {
  ...
  (isTRUE(all.equal(result$analysis$n, round(result$analysis$n))))

  # Check for analysis event
  (isTRUE(all.equal(result$analysis$event, round(result$analysis$event), tolerance = 1e-6)))
...
check_fixed_design_summary <- function(summary_x) {
  ...
  (isTRUE(all.equal(ncol(summary_x), 8)))
...
assert("to_integer works correctly for different fixed design types", {
  ...
  for (design in designs) {
    ...
    (isTRUE(all.equal(x$design, design$name)))
