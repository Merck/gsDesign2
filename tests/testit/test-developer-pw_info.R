assert("Output column of n matches with expected_accrual with Inf in the fail_rate", {
  ...
  x2 <- expected_accrual(enroll_rate = enroll_rate,
                         time = my_time)
  (isTRUE(all.equal(cumsum(x1$n), x2)))
  ...
  (isTRUE(all.equal(sum(x1$n[x1$time == 2]), expected_accrual(enroll_rate = enroll_rate, time = 2))))
  (isTRUE(all.equal(sum(x1$n[x1$time == 10]), expected_accrual(enroll_rate = enroll_rate, time = 10))))
})
...
assert("When control median changes but HR remain unchanged", {
  x <- pw_info(
  ...
  (isTRUE(all.equal(sum(x$n), 20 * 12)))
})
