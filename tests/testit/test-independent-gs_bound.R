assert("compare with results from gsDesign, 3 analyses, equal IA timing", {
  x <- gsDesign::gsSurv(
  ...
  (isTRUE(all.equal(unlist(test1[1], use.names = FALSE), xbound[, 1], tolerance = 0.05)))
  (isTRUE(all.equal(unlist(test1[2], use.names = FALSE), xbound[, 2], tolerance = 0.05)))
})
...
assert("compare with results from gsDesign, 3 analyses, unequal IA timing", {
  ...
  (isTRUE(all.equal(unlist(test2[1], use.names = FALSE), ybound[, 1], tolerance = 0.05)))
  (isTRUE(all.equal(unlist(test2[2], use.names = FALSE), ybound[, 2], tolerance = 0.05)))
})
