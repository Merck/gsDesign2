assert("gsDesign2:::h1() returns results as expected ", {
  # the design
  ...
  (all_equal(as.numeric(c(upper.null, upper.alt)), x$upper$prob[1, ], tolerance = 0.0001))
  (all_equal(as.numeric(c(lower.null, lower.alt)), x$lower$prob[1, ], tolerance = 0.0001))
})
...
assert("gsDesign2:::h1() returns probability almost zero for extreme case", {
  ...
  (all_equal(as.numeric(c(exmtest1, exmtest2)), c(0, 0), tolerance = 0.0001))
})
