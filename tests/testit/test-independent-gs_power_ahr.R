...
assert("under same number of events, compare the power", {
  x <- res$x
  y <- res$y
  ...
  (isTRUE(all.equal(out$bound$probability[1:2], y$Efficacy[c(5, 10)], tolerance = 0.02)))
})
...
assert("under same power setting, compare the number of events", {
  ...
  (isTRUE(all.equal(out$analysis$event[1:2], x$n.I, tolerance = 0.002)))
})
