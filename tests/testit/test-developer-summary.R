...
assert("summary.gs_design() accepts same-length vectors for analysis_vars and analysis_decimals", {
  ...
  (observed %==% paste0("Analysis: 1 Time: 12 N: 707.3 Events: 160.4 AHR: 0.81 Information fraction: ", round(x$analysis$event[1]/ max(x$analysis$event), 2)))
  ...
  observed <- x |>
    ...
    attr("groups") |>
    extract_summary_analysis()
  (observed %==% "Analysis: 1 Time: 12 N: 707 Events: 160 AHR: 0.8108 Information fraction: 0.4191")
  ...
  observed <- x |>
    ...
    attr("groups") |>
    extract_summary_analysis()
  (observed %==% "Analysis: 1 AHR: 0.8108 Information fraction: 0.4191")
  ...
  observed <- x |>
    ...
    attr("groups") |>
    extract_summary_analysis()
  (observed %==% "Analysis: 1 Information fraction: 0.4191 AHR: 0.8108 Events: 160 N: 707 Time: 12")
...
assert("summary.gs_design() accepts a named vector for analysis_decimals", {
  ...
  observed <- x |>
    ...
    attr("groups") |>
    extract_summary_analysis()
  (observed %==% paste0("Analysis: 1 Time: 12 N: 707.3 Events: 160.4 AHR: 0.8108 Information fraction: ", round(x$analysis$event[1]/ max(x$analysis$event), 4)))
  ...
  observed <- x |>
    ...
    attr("groups") |>
    extract_summary_analysis()
  (observed %==% paste0("Analysis: 1 Events: 160.4 AHR: 0.8108 Information fraction: ", round(x$analysis$event[1]/max(x$analysis$event), 4)))
  ...
  observed <- x |>
    ...
    attr("groups") |>
    extract_summary_analysis()
  (observed %==% paste0("Analysis: 1 Information fraction: ", round(x$analysis$event[1]/max(x$analysis$event), 4), " AHR: 0.8108 Events: 160.4"))
...
assert("The full alpha is correctly carried over", {
  a_level <- 0.02
  x <- gs_power_ahr(
  ...
  (isTRUE(all.equal(attributes(observed)$full_alpha, a_level)))
})

# Maintain previous behavior
assert("summary.gs_design() accepts same-length vectors for col_vars and col_decimals", {
  x <- gs_design_ahr()
...
assert("summary.gs_design() outputs spending time correctly", {
  x <- gs_design_ahr(info_frac = 1:3/3) |> summary(display_spending_time = TRUE)
  ("Spending time" %in% names(x))
