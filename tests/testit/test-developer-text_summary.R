assert("Text summary of a 1-sided design", {
  x <- gs_design_ahr(info_frac = 1:3 / 3, test_lower = FALSE) |> to_integer()

  (text_summary(x) %==% "One-sided group sequential design with 3 analyses, time-to-event outcome with sample size 490 and 301 events, 90 percent power, 2.5 percent (1-sided) Type I error to detect hazard ratio of 0.9 during the first 3 months and 0.6 thereafter. Enrollment and total study durations are assumed to be 14 and 36.1 months, respectively. Efficacy bounds derived using a Lan-DeMets O'Brien-Fleming approximation spending function (no parameters).")
})

assert("Text summary of a 2-sided symmetric design", {
  x <- gs_design_ahr(
    info_frac = 1:3 / 3,
  ...
  (text_summary(x) %==% "Symmetric two-sided group sequential design with 3 analyses, time-to-event outcome with sample size 490 and 301 events, 90 percent power, 2.5 percent (1-sided) Type I error to detect hazard ratio of 0.9 during the first 3 months and 0.6 thereafter. Enrollment and total study durations are assumed to be 14 and 36.1 months, respectively. Bounds derived using a  Lan-DeMets O'Brien-Fleming approximation spending function (no parameters).")
})

assert("Text summary of a asymmetric 2-sided design with beta-spending and non-binding futility bound", {
  x <- gs_design_ahr(
    info_frac = 1:3 / 3,
    alpha = 0.025,
    beta = 0.1,
...
assert("If there are >5 pieces of HRs, we provide a brief summary of HR.", {
  x <- gs_design_ahr(
  ...
  (text_summary(x) %==% "One-sided group sequential design with 3 analyses, time-to-event outcome with sample size 290.5 and 218 events, 90 percent power, 2.5 percent (1-sided) Type I error to detect piecewise hazard ratio. Enrollment and total study durations are assumed to be 14 and 36 months, respectively. Efficacy bounds derived using a Lan-DeMets O'Brien-Fleming approximation spending function (no parameters).")
})

assert("Text summary of a fixed design created with gs_design_ahr()", {
  x <- gs_design_ahr(
    upper = gs_b,
    lower = gs_b,
    upar = qnorm(1 - 0.025),
...
assert("Text summary for two strata with 6 pieces non-proportional hazards", {
  x <- gs_design_ahr(
    enroll_rate = define_enroll_rate(
      stratum = c("A", "B"),
      duration = c(12, 12),
...
assert("Text summary for >2 strata with >=3 pieces non-proportional hazards", {
  x <- gs_design_ahr(
    enroll_rate = define_enroll_rate(
      stratum = c("A", "B", "C"),
      duration = c(12, 12, 12),
...
assert("Text summary for two strata with piecewise enrollment rates", {
