assert("The IA nominal p-value is the same as the IA alpha spending.", {
  ...
  (isTRUE(all.equal(x$bound$`nominal p`[1], gsDesign::sfLDOF(alpha = 0.025, t = 18 / 30)$spend[1])))
})
...
assert("Validate the sample size rounding under equal randomization (1:1) for TTE endpoint. -- GSD", {
  ...
  (isTRUE(all.equal(ceiling(x$analysis$n[2] / 2) * 2, y1$analysis$n[2])))
  # test the event at FA is rounded up
  (isTRUE(all.equal(ceiling(x$analysis$event[2]), y1$analysis$event[2])))
  # test the event at IA is rounded
  (isTRUE(all.equal(round(x$analysis$event[1], 0), y1$analysis$event[1])))
  ...
  # test the sample size at FA is rounded up and multiple of 2
  (isTRUE(all.equal(round(x$analysis$n[2] / 2, 0) * 2, y2$analysis$n[2])))
  # test the event at FA is rounded
  (isTRUE(all.equal(round(x$analysis$event[2], 0), y2$analysis$event[2])))
  # test the event at IA is rounded
  (isTRUE(all.equal(round(x$analysis$event[1], 0), y2$analysis$event[1])))
...
assert("Validate the sample size rounding under unequal randomization (3:2) for TTE endpoint. -- GSD", {
  ...
  (isTRUE(all.equal(ceiling(x$analysis$n[2]), y1$analysis$n[2])))
  ...
  (isTRUE(all.equal(ceiling(x$analysis$n[2] / 5) * 5, y2$analysis$n[2])))
  # test the FA events is rounded up
  (isTRUE(all.equal(ceiling(x$analysis$event[2]), y2$analysis$event[2])))
  # test the IA events is rounded
  (isTRUE(all.equal(round(x$analysis$event[1], 0), y2$analysis$event[1])))
  ...
  (isTRUE(all.equal(round(x$analysis$n[2]), y3$analysis$n[2])))
  ...
  (isTRUE(all.equal(round(x$analysis$n[2] / 5, 0) * 5, y4$analysis$n[2])))
  # test the FA events is rounded
  (isTRUE(all.equal(round(x$analysis$event[2], 0), y2$analysis$event[2])))
  # test the IA events is rounded
  (isTRUE(all.equal(round(x$analysis$event[1], 0), y2$analysis$event[1])))
...
assert("Validate the sample size rounding under equal randomization (1:1) for binary endpoint. -- GSD", {
  ...
  (isTRUE(all.equal(c(round(x$analysis$n[1:2], 0), ceiling(x$analysis$n[3] / 2) * 2), y1$analysis$n)))
  (isTRUE(all.equal(c(round(x$analysis$n[1:2], 0), round(x$analysis$n[3] / 2, 0) * 2), y2$analysis$n)))
...
assert("Validate the sample size rounding under unequal randomization (3:2) for binary endpoint. -- GSD", {
  ...
  y1 <- x |> to_integer(round_up_final = TRUE)
  (isTRUE(all.equal(ceiling(x$analysis$n[3]), y1$analysis$n[3])))

  # ceiling the sample size at FA, and is a multiplier of 5
  y2 <- x |> to_integer(round_up_final = TRUE, ratio = 4)
  (isTRUE(all.equal(ceiling(x$analysis$n[3] / 4) * 4, y2$analysis$n[3])))

  # round the sample size at FA, but may not a multiplier of 5
  y3 <- x |> to_integer(round_up_final = FALSE)
  (isTRUE(all.equal(round(x$analysis$n[3], 0), y3$analysis$n[3])))

  # round the sample size at FA, and is a multiplier of 5
  y4 <- x |> to_integer(round_up_final = FALSE, ratio = 4)
  (isTRUE(all.equal(round(x$analysis$n[3] / 5, 0) * 5, y4$analysis$n[3])))
...
assert("Validate the sample size rounding under equal randomization (1:1) for TTE endpoint -- fixed design.", {
  ...
  (isTRUE(all.equal(ceiling(x$analysis$n / 2) * 2, y1$analysis$n)))
  (isTRUE(all.equal(round(x$analysis$n / 2, 0) * 2, y2$analysis$n)))
...
assert("Validate the sample size rounding under unequal randomization (3:2) for TTE endpoint.", {
  ...
  (isTRUE(all.equal(ceiling(x$analysis$n), y1$analysis$n)))
  # test the event is rounded up
  (isTRUE(all.equal(ceiling(x$analysis$event), y1$analysis$event)))
  ...
  (isTRUE(all.equal(ceiling(x$analysis$n / 5) * 5, y2$analysis$n)))
  # test the event is rounded up
  (isTRUE(all.equal(ceiling(x$analysis$event), y2$analysis$event)))
  ...
  (isTRUE(all.equal(round(x$analysis$n), y3$analysis$n)))
  # test the event is rounded
  (isTRUE(all.equal(round(x$analysis$event, 0), y3$analysis$event)))
  ...
  (isTRUE(all.equal(round(x$analysis$n / 5, 0) * 5, y4$analysis$n)))
  # test the event is rounded
  (isTRUE(all.equal(ceiling(x$analysis$event), y4$analysis$event)))
...
assert("verify the crossing prob of a MB design at IA1 under null", {
  ...
  (isTRUE(all.equal((x$bounds |> filter(bound == "upper", analysis == 1))$probability0, sfLDOF(alpha = .025, t = x$analysis$info_frac0)$spend[1])))
})
...
assert("The attribute `uninteger_is_from` matches the input design object", {
  for (design_func in c("gs_design_ahr", "gs_design_rd", "gs_design_wlr")) {
    ...
    (attr(x, "uninteger_is_from") %==% design_func)
  }
  ...
  for (power_func in c("gs_power_ahr", "gs_power_rd", "gs_power_wlr")) {
    ...
    x <- to_integer(x)
    (attr(x, "uninteger_is_from") %==% power_func)
  }
})
