...
assert("Correct computation of blinded AHR and information adjustment", {
  ...
  (isTRUE(all.equal(result$event, expected_event)))
  (isTRUE(all.equal(result$ahr, expected_ahr, tolerance = 0.001)))
  (isTRUE(all.equal(result$theta, expected_theta, tolerance = 0.001)))
  (isTRUE(all.equal(result$info0, expected_info0)))
})
...
assert("ahr_blinded computes theta with constant hazard ratios correctly", {
  ...
  (isTRUE(all.equal(result$theta, 0)))
})
...
assert("ahr_blinded handles zero events", {
  ...
  (isTRUE(all.equal(result$event, 0)))
  (is.nan(result$ahr))
  (is.nan(result$theta))
  (isTRUE(all.equal(result$info0, 0)))
})
...
assert("ahr_blinded handles all events in the first interval", {
  ...
  (isTRUE(all.equal(result$theta, expected_theta)))
  (isTRUE(all.equal(result$event, sum(surv[, "status"]))))
})
...
assert("ahr_blinded handles very high randomization ratio", {
  ...
  (isTRUE(all.equal(result$info0, 0, tolerance = 0.05)))
})
