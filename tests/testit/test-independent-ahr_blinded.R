...
assert("Correct computation of blinded AHR and information adjustment", {
  ...
  (all.equal(result$event, expected_event))
  (all.equal(result$ahr, expected_ahr, tolerance = 0.001))
  (all.equal(result$theta, expected_theta, tolerance = 0.001))
  (all.equal(result$info0, expected_info0))
})
...
assert("ahr_blinded handles zero events", {
  ...
  (all.equal(result$event, 0))
  (is.nan(result$ahr))
  (is.nan(result$theta))
  (all.equal(result$info0, 0))
})
...
assert("ahr_blinded handles all events in the first interval", {
  ...
  (all.equal(result$theta, expected_theta))
  (all.equal(result$event, sum(surv[, "status"])))
})
