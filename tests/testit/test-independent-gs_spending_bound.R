assert("compare gs_spending_bound with gsDesign results with equal IA timing for upper and lower bound", {
  x <- gsDesign::gsSurv(
    k = 3,
    test.type = 4,
    alpha = 0.025,
  ...
  (isTRUE(all.equal(as.numeric(test1), x$upper$bound, tolerance = 0.0001)))
  (isTRUE(all.equal(as.numeric(test2), x$lower$bound, tolerance = 0.0001)))
})
...
assert("compare gs_spending_bound with gsDesign results with unequal IA timing for upper and lower bound", {
  ...
  (isTRUE(all.equal(as.numeric(test3), y$upper$bound, tolerance = 0.0001)))
  (isTRUE(all.equal(as.numeric(test4), y$lower$bound, tolerance = 0.0001)))
})
