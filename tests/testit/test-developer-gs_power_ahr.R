...
assert("calendar based cut", {
  ...
  (all.equal(x1$analysis$time, x2$Time[x2$Bound == "Upper"]))
  (all.equal(x1$analysis$event, x2$Events[x2$Bound == "Upper"]))
  (all.equal(x1$bound$z[x1$bound$bound == "upper"], x2$Z[x2$Bound == "Upper"]))
  (all.equal(x1$bound$probability[x1$bound$bound == "upper"], x2$Probability[x2$Bound == "Upper"]))
  (all.equal(x1$analysis$ahr, x2$AHR[x2$Bound == "Upper"]))
  (all.equal(x1$analysis$theta, x2$theta[x2$Bound == "Upper"]))
  (all.equal(x1$analysis$info, x2$info[x2$Bound == "Upper"]))
  (all.equal(x1$analysis$info0, x2$info0[x2$Bound == "Upper"]))
  (all.equal(x1$analysis$time, x2$Time[x2$Bound == "Lower"]))
  (all.equal(x1$analysis$event, x2$Events[x2$Bound == "Lower"]))
  (all.equal(x1$bound$z[x1$bound$bound == "lower"], x2$Z[x2$Bound == "Lower"]))
  (all.equal(x1$bound$probability[x1$bound$bound == "lower"], x2$Probability[x2$Bound == "Lower"]))
  (all.equal(x1$analysis$ahr, x2$AHR[x2$Bound == "Lower"]))
  (all.equal(x1$analysis$theta, x2$theta[x2$Bound == "Lower"]))
  (all.equal(x1$analysis$info, x2$info[x2$Bound == "Lower"]))
  (all.equal(x1$analysis$info0, x2$info0[x2$Bound == "Lower"]))
})
...
assert("Use default lower and lpar but set test_lower is FALSE", {
  ...
  (all.equal(unique(x$bound$bound), "upper"))
})
