assert("default parameter", {
  ...
  (all.equal(x1$bound$probability[x1$bound$bound == "upper"], x2$Probability[x2$Bound == "Upper"], tolerance = 2e-6))
  (all.equal(x1$analysis$ahr, x2$AHR[x2$Bound == "Upper"], tolerance = 3e-7))
  (all.equal(x1$analysis$theta, x2$theta[x2$Bound == "Upper"], tolerance = 5e-7))
...
assert("event based cut", {
  ...
  (all.equal(x1$analysis$time, x2$Time[x2$Bound == "Upper"], tolerance = 2e-7))
  (all.equal(x1$analysis$event, x2$Events[x2$Bound == "Upper"], tolerance = 2e-7))
  (all.equal(x1$bound$z[x1$bound$bound == "upper"], x2$Z[x2$Bound == "Upper"], tolerance = 1e-7))
  (all.equal(x1$bound$probability[x1$bound$bound == "upper"], x2$Probability[x2$Bound == "Upper"], tolerance = 3e-7))
  (all.equal(x1$analysis$ahr, x2$AHR[x2$Bound == "Upper"], tolerance = 3e-7))
  (all.equal(x1$analysis$theta, x2$theta[x2$Bound == "Upper"], tolerance = 1e-7))
  (all.equal(x1$analysis$info, x2$info[x2$Bound == "Upper"], tolerance = 2e-7))
  ...
  (all.equal(x1$analysis$info0, x2$info0[x2$Bound == "Lower"], tolerance = 3e-6))
})
...
assert("calendar + event based cut", {
  ...
  (all.equal(x1$analysis$time, x2$Time[x2$Bound == "Upper"], tolerance = 2e-6))
  (all.equal(x1$analysis$event, x2$Events[x2$Bound == "Upper"], tolerance = 3e-6))
  (all.equal(x1$bound$z[x1$bound$bound == "upper"], x2$Z[x2$Bound == "Upper"], tolerance = 1e-6))
  (all.equal(x1$bound$probability[x1$bound$bound == "upper"], x2$Probability[x2$Bound == "Upper"], tolerance = 4e-7))
  (all.equal(x1$analysis$ahr, x2$AHR[x2$Bound == "Upper"], tolerance = 3e-7))
  (all.equal(x1$analysis$theta, x2$theta[x2$Bound == "Upper"], tolerance = 3e-7, scale = 1))
  (all.equal(x1$analysis$info, x2$info[x2$Bound == "Upper"], tolerance = 3e-6))
