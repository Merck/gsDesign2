...
assert("Validate the function based on simple calculation", {
  ...
  (isTRUE(all.equal(as.numeric(pckwlrn), wlrn, tolerance = 0.0001)))
  (isTRUE(all.equal(as.numeric(fhwei), pckfhwei, tolerance = 0.0001)))
  (isTRUE(all.equal(as.numeric(FH00wt), 1, tolerance = 0)))
})
