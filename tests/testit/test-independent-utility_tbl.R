assert("test round_df", {
  ...
  (isTRUE(all.equal(tbl_new$x1, tbl$x1)))
  (isTRUE(all.equal(tbl_new$x2, round(x2, 2))))
  (isTRUE(all.equal(tbl_new$x3, round(x3, 2))))

  tbl_new <- round_df(tbl, digits = c(1, 1, 2))
  (isTRUE(all.equal(tbl_new$x1, tbl$x1)))
  (isTRUE(all.equal(tbl_new$x2, round(x2, 1))))
  (isTRUE(all.equal(tbl_new$x3, round(x3, 2))))
})
...
assert("test table_ab", {
  ...
  (isTRUE(all.equal(tbl$Index, c(
    paste0("Index: 1 a1: ", round(1.1234, 2), " a2: text 1"),
    paste0("Index: 2 a1: ", round(5.6789, 2), " a2: text 2")
  ))))
  (isTRUE(all.equal(tbl$b1, b$b1)))
  (isTRUE(all.equal(tbl$b2, b$b2)))
  (isTRUE(all.equal(tbl$b3, b$b3)))
})
