...
assert("Independent Tests - Expect equal with mvtnorm for efficacy and futility bounds", {
  ...
  pb <- 1 - pnorm(b$root)
  (isTRUE(all.equal(test1$z, c(qnorm(1 - alpha_ia), b$root), tolerance = 0.001)))
  (isTRUE(all.equal(test1$probability, cumsum(c(b_ia$spend, pb)), tolerance = 0.001)))
  beta_t <- 0.02
  ...
  pa <- pnorm(a$root)
  (isTRUE(all.equal(test2$z, c(qnorm(beta_ia), a$root), tolerance = 0.001)))
  (isTRUE(all.equal(test2$probability, cumsum(c(a_ia$spend, pa)), tolerance = 0.001)))
})
...
assert("Expect equal with gsDesign::gsProbability outcome for efficacy bounds", {
  ...
  (isTRUE(all.equal(x, y)))
  (isTRUE(all.equal(x$z[x$bound == "upper"], z$upper$bound, tolerance = 1e-5)))
  (isTRUE(all.equal(x$probability[x$bound == "upper"], cumsum(z$upper$prob), tolerance = 1e-5)))
})
