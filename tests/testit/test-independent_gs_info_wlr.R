...
assert("Validate the function based on examples with individual functions", {
  ...
  # FH(0,1)
  (all_equal(as.numeric(fh01$n), rep(N01, 3), tolerance = 1))
  (all_equal(as.numeric(fh01$event), evt01, tolerance = 1))
  (all_equal(as.numeric(fh01$delta), -delta01, tolerance = .01))
  (all_equal(as.numeric(fh01$sigma2), sigma201, tolerance = .01))
  (all_equal(as.numeric(fh01$theta), theta01, tolerance = .2))
  (all_equal(as.numeric(fh01$info), info01, tolerance = 0.3))
})
