...
assert("Validate the function based on examples with individual functions", {
  ...
  (all.equal(as.numeric(fh01$delta), -delta01, tolerance = .01))
  (all.equal(as.numeric(fh01$sigma2), sigma201, tolerance = .01))
  (all.equal(as.numeric(fh01$theta), theta01, tolerance = .2))
