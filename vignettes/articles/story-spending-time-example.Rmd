...
Traditionally @lan1983discrete, spending has been done according to targeting a specific number of events for an outcome at the end of the trial.
However, for delayed treatment effect scenarios there is substantial literature (e.g., @lin2020alternative, @roychoudhury2021robust) documenting the importance of adequate follow-up duration in addition to requiring an adequate number of events under the traditional proportional hazards assumption.
...
The fact that spending does not need to correspond to information fraction was perhaps first raised by @lan1989group where calendar-time spending was discussed.
However, we note that @proschan2006statistical has raised other scenarios where spending alternatives are considered.
...
- Spending with a common spending time across multiple hypotheses; e.g., in the multiple population example, spending in the overall population at the same rate as in the biomarker positive subgroup regardless of event counts over time in the overall population. This is consistent with @follmann1994monitoring as applied when multiple experimental treatments are compared to a common control. Spending time in this case corresponds to the approach of @fleming1984designs where fixed incremental spending is set for a potentially variable number of interim analyses.
...
Code blocks can be unfolded for those interested in implementation.
...
The approach taken uses an average-hazard ratio approach for approximating treatment effect as in @mukhopadhyay2020statistical and the asymptotic group sequential theory of @tsiatis1982repeated.
...
We assume the efficacy bound uses the @lan1983discrete spending function approximating an O'Brien-Fleming bound.
...
The first demonstrates calendar spending as in @lan1989group.
The second is a basically the method of @fleming1984designs where a fixed incremental spend is used for a potentially variable number of interim analyses, with the final bound computed based on the unspent one-sided Type I error assigned to a hypothesis.
...
As noted, this method was proposed by @fleming1984designs.
...
We consider a simple case where we use the method of @maurer2013multiple to test both in the overall population and in a biomarker subgroup for the same endpoint.
...
For each case, we will assume the total enrollment rate of `r round(sum(planned_enroll_rate$rate), 1)` per month as planned above.
We also assume that we enroll until the targeted biomarker positive subgroup enrollment of
`r ceiling(sum(positive$enroll_rate$rate * positive$enroll_rate$duration))` from above is achieved, regardless of the overall enrollment.
...
We gave two options to this:
    - Use a fixed, small incremental $\alpha$-spend at each interim such as proposed by @fleming1984designs with a variable number of interim analyses to ensure adequate follow-up.
    - Use the minimum of planned and actual spending at interim analyses.

When implementing the @fleming1984designs approach, we also suggested a simple approach to futility that may be quite useful practically in a scenario with a potentially delayed onset of treatment effect.
...
Using a common spending time for all hypotheses with a common interim analysis strategy as advocated by @follmann1994monitoring can be helpful to implement spending so that all hypotheses have adequate $\alpha$ to spend at the final analysis and also to ensure full utilization of $\alpha$-spending. We suggested again using the minimum of planned and actual spending at interim analysis. Spending can be based on a key hypothesis (e.g., the biomarker positive population) or the minimum spending time among all hypotheses being tested.
Taking advantage of know correlations to ensure full $\alpha$ utilization in multiple hypothesis testing is also more simply implemented with this strategy @anderson2022unified.

In summary, we have illustrated both the motivation and the illustration of the spending time approach through examples we have commonly encountered. Approaches suggested included an implementation of @fleming1984designs with a fixed incremental $\alpha$-spend at each interim analysis as well as the use of the minimum of planned and actual spending at interim analyses.
