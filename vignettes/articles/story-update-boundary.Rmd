...
For the design, we have efficacy bounds at both the IA and FA. The spending function used is the @lan1983discrete spending function with a total alpha of `r alpha`, which approximates an O'Brien-Fleming bound.
...
x |>
  ...
  tab_style(style = list(cell_fill(color = "lightcyan"),
            ...
            locations = cells_body(columns = Z,
                                   rows = Bound == "Efficacy")) |>
...
```{r}
x_updated <- gs_power_npe(theta = 0,
                          # info
...
x_updated |>
  ...
  tab_style(style = list(cell_fill(color = "lightcyan"),
            ...
            locations = cells_body(columns = z,
                                   rows = bound == "upper")) |>
  tab_footnote(footnote = "Crossing pbability under H0.",
               locations = cells_column_labels(columns = probability))
```
...
x_updated |>
  ...
  tab_style(style = list(cell_fill(color = "lightcyan"),
                         cell_text(weight = "bold")),
            locations = cells_body(columns = probability,
                                   rows = bound == "upper")) |>
  tab_style(style = list(cell_fill(color = "#F9E3D6"),
                         cell_text(weight = "bold")),
            locations = cells_body(columns = probability,
