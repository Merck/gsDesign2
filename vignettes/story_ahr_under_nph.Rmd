...
enroll_rate$rate <- ceiling(target_event) / avehr$Events * enroll_rate$rate
avehr <- AHR(
  enroll_rate = enroll_rate,
  fail_rate = fail_rate,
  total_duration = as.numeric(total_duration)
...
sampleSize <- ceiling(sum(enroll_rate$rate * enroll_rate$duration) / 2) * 2
sampleSize
```
...
# save(results1, file = './fixtures/results1.Rdata') # Save the data
```
...
enroll_rate <- enroll_rate %>% mutate(rate = target_event / ahr2$Events * rate)
