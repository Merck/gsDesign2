...
hr <- do.call(
  ...
  lapply(
    ...
    function(x){
      AHR(enroll_rate = enroll_rate %>% filter(Scenario == x), 
          fail_rate = fail_rate %>% filter(Scenario == x),
          total_duration = c(.001, seq(4, 44, 4))) %>% mutate(Scenario = x)
    }))
```
...
ss_ahr_fixed <- do.call(
  ...
  lapply(c("PH", "Shorter delayed effect", "Longer delayed effect", "Crossing"), 
         function(x) {
           xx <- gs_design_ahr(enroll_rate = enroll_rate %>% filter(Scenario == x),
                               fail_rate = fail_rate %>% filter(Scenario == x),
                               analysis_time = 36,
                               upper = gs_b, 
                               upar = qnorm(.975),
...
do.call(
  ...
  lapply(c(24, 30, 36, 42), 
         function(x){
           ans <- gs_design_ahr(enroll_rate = enroll_rate %>% filter(Scenario == "Shorter delayed effect"),
...
for(g in c("PH", "Shorter delayed effect", "Longer delayed effect", "Crossing")){
    events_by_time_period <- rbind(
      ...
      AHR(enroll_rate = enroll_rate %>% filter(Scenario == g),
          fail_rate = fail_rate %>% filter(Scenario == g),
          total_duration = c(12, 20, 28, 36), simple = FALSE) %>% mutate(Scenario = g))
}
```
...
NPHasymmetric <- gs_design_ahr(enroll_rate = enroll_rate,
                               fail_rate = fail_rate,
...
do.call(
  ...
  lapply(
    ...
    function(x){
      ...
      yy <- gs_power_ahr(enroll_rate = enroll_rate %>% filter(Scenario == x),
                         fail_rate = fail_rate %>% filter(Scenario == x), 
                         events = NULL,
                         analysis_time = c(12, 20, 28, 36),
                         upper = gs_spending_bound, 
...
do.call(
  ...
  lapply(
    c("PH", "Shorter delayed effect","Longer delayed effect", "Crossing"), 
    function(x){
      gs_design_wlr(enroll_rate = enroll_rate %>% filter(Scenario == x), 
                    fail_rate = fail_rate %>% filter(Scenario == x), 
