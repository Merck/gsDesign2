...
expected_event(enroll_rate = enroll_rate, fail_rate = fail_rate, total_duration = total_duration, simple = FALSE)
```
...
```{r}
enroll_rate <- tibble(duration = 10, rate = 10)
fail_rate <- tibble(duration = c(20, 80), fail_rate = c(log(2)/6, log(2)/4), dropout_rate = .01)
total_duration <- 22
...
try(expected_event(enroll_rate = enroll_rate, fail_rate = fail_rate, total_duration = total_duration, simple = FALSE))
```
...
```{r}
fail_rate <- tibble(duration = c(20, 80), failRate = c(0.1, 0.2), dropout_rate = .01)
...
```{r}
df_1 <- tibble(startEnroll = c(0, cumsum(enroll_rate$duration)),
...
```{r}
df_2 <- tibble(endFail = cumsum(fail_rate$duration),
...
df_2 %>% 
  gt() %>%
  tab_header(title = "df_2", subtitle = "Updated by adjusting the analysis time and fail_rate duration") %>% 
  tab_footnote(footnote = "The time when the failure changes.", locations = cells_column_labels("endFail")) %>%
  tab_footnote(footnote = "The time from endFail to the analysis time.", locations = cells_column_labels("startEnroll")) %>% 
  tab_footnote(footnote = "The failure rates", locations = cells_column_labels("failRate")) %>% 
  tab_footnote(footnote = "The dropout rates", locations = cells_column_labels("dropoutRate"))
...
```{r}
df <- df %>% mutate(endEnroll = lag(startEnroll, default = as.numeric(total_duration)),
