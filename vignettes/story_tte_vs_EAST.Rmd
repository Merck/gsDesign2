...
- `rpact::getDesignGroupSequential()` as used below seems, also to use exact informaiton rates similar to `gsDesign` and `gsDesign2`.
...
os_rpact <-  os_scen |>
  dplyr::mutate(
    rpact_out = purrr::map(
      alevel, function(x,...){
        ...
        out <- 
          getDesignGroupSequential(sided = 1, 
                                   alpha = x,
                                   informationRates = c(timing_east, 1), 
                                   typeOfDesign = "asOF")
      ...
      timing_east = timing_east, failrate_os = failrate_os, hr1_os = hr1_os, 
