...
run_simulation <- function(N, weight_method, 
  ...
  n_s1 <- ceiling(n * prevalence_ratio[1] / sum(prevalence_ratio)) * info_frac
  n_s2 <- ceiling(n * prevalence_ratio[2] / sum(prevalence_ratio)) * info_frac
  n_s3 <- ceiling(n * prevalence_ratio[3] / sum(prevalence_ratio)) * info_frac
  ...
  for (simu in 1:n_sim) {
    ...
    x <- tibble::tibble(
      ...
      IF = rep(info_frac, 3),
      sample_size_e = c(n_s1 / 2, n_s2 / 2, n_s3 / 2),
      sample_size_c = n_e,
      ...
      mutate(
        p_pool = (n_c + n_e) / (sample_size_c + sample_size_e),
        sigma_h0 = sqrt(p_pool * (1 - p_pool) * (1 / sample_size_c + 1 / sample_size_e)),
        sigma_h1 = sqrt(p_c * (1 - p_c) / sample_size_c + p_e * (1 - p_e) / sample_size_e)
      )
    ...
    sum_weight <- x %>%
      ...
      summarize(
        sum_invar_H0 = sum(1 / sigma_h0^2),
        sum_invar_H1 = sum(1 / sigma_h1^2),
        sum_ss = sum((sample_size_e * sample_size_c) / (sample_size_e + sample_size_c))
      ) %>%
      ungroup()
    ...
    suppressMessages(
      x <- x %>%
        ...
        summarise( # weighted risk difference
          ...
          info0_ss = 1 / sum(weight_ss^2 * p_pool * (1 - p_pool) * (1 / sample_size_e + 1 / sample_size_c))
        ) %>%
        ungroup()
    )
    ...
    if (integration == "pmvtnorm") {
      ...
      if (weight_method == "invar_H0") {
        if (under_H0) {
          info_frac_sim <- x$info0_invar_H0 / x$info0_invar_H0[3]
          mean_sim <- x$rd_invar_H0 * sqrt(x$info0_invar_H0)
        } else {
          info_frac_sim <- x$info_invar_H1 / x$info_invar_H1[3]
      ...
      if (weight_method == "invar_H1") {
        if (under_H0) {
          info_frac_sim <- x$info0_invar_H1 / x$info0_invar_H1[3]
      ...
      if (weight_method == "ss") {
        if (under_H0) {
          info_frac_sim <- x$info0_ss / x$info0_ss[3]
          mean_sim <- x$rd_ss * sqrt(x$info0_ss)
        } else {
          info_frac_sim <- x$info_ss / x$info_ss[3]
      ...
      p <- pmvnorm(
        ...
        corr = matrix(
          c(
            1, sqrt(info_frac_sim[1] / info_frac_sim[2]), sqrt(info_frac_sim[1] / info_frac_sim[3]),
            sqrt(info_frac_sim[1] / info_frac_sim[2]), 1, sqrt(info_frac_sim[2] / info_frac_sim[3]),
            sqrt(info_frac_sim[1] / info_frac_sim[3]), sqrt(info_frac_sim[2] / info_frac_sim[3]), 1
          ),
          nrow = 3, byrow = TRUE
        ),
        sigma = NULL,
  ...
  lapply(
    ...
    function(x) {
      return(run_simulation(
        ...
        info_frac = IF, integration = "gs_power_npe"
      ))
    }
  )
)
