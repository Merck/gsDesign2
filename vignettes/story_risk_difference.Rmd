...
tibble(
  `info_scale = 0` = y_0$info0[1] / x$info0[1],
...
tibble::tibble(
  ...
  cols_label(
    gsDesign2_info_scale_0 = "info_scale = 0",
...
tibble(
  `info_scale = 0` = y_gs0$info0 / x_gs$info0[3],
...
x %>%
  ...
  gt::tab_footnote(
    footnote = "info_invar_H0 = the statistical information under H1 per stratum per analysis calculated by INVAR by using variance under H0.",
    locations = gt::cells_column_labels(columns = info_invar_H0)
  ) %>%
  gt::tab_footnote(
    footnote = "info_invar_H1 = the statistical information under H1 per stratum per analysis calculated by INVAR by using variance under H0.",
    locations = gt::cells_column_labels(columns = info_invar_H1)
  ) %>%
  gt::tab_footnote(
    footnote = "info_ss = the statistical information under H1 per stratum per analysis calculated by SS.",
    locations = gt::cells_column_labels(columns = info_ss)
  ) %>%
  gt::tab_footnote(
    footnote = "info0_invar_H0 = the statistical information under H0 per stratum per analysis calculated by INVAR by using variance under H0.",
    locations = gt::cells_column_labels(columns = info0_invar_H0)
  ) %>%
  gt::tab_footnote(
    footnote = "info0_invar_H1 = the statistical information under H0 per stratum per analysis calculated by INVAR by using variance under H0.",
    locations = gt::cells_column_labels(columns = info0_invar_H1)
  ) %>%
  gt::tab_footnote(
    footnote = "info0_ss = the statistical information under H0 per stratum per analysis calculated by SS.",
...
```{r}
ans <- tibble::tibble(
  `INVAR using variance under H0 with info_scale = 0` = x_invar0$analysis$N[1:3],
  `INVAR using variance under H0 with info_scale = 1` = x_invar1$analysis$N[1:3],
  `INVAR using variance under H0 with info_scale = 2` = x_invar2$analysis$N[1:3],
  `INVAR using variance under H1 with info_scale = 0` = x_invar_h1_0$analysis$N[1:3],
  `INVAR using variance under H1 with info_scale = 1` = x_invar_h1_1$analysis$N[1:3],
  `INVAR using variance under H1 with info_scale = 2` = x_invar_h1_2$analysis$N[1:3],
...
ans %>%
  ...
  gt::tab_header(title = "Sample size calculated by INVAR and SS") %>%
  gt::tab_spanner(
    label = "Inverse variance weighting with H0 variance",
    columns = c("INVAR using variance under H0 with info_scale = 0", 
                "INVAR using variance under H0 with info_scale = 1", 
                "INVAR using variance under H0 with info_scale = 2")
  ) %>%
  gt::tab_spanner(
    label = "Inverse variance weighting with H1 variance",
    columns = c("INVAR using variance under H1 with info_scale = 0", 
                "INVAR using variance under H1 with info_scale = 1", 
                "INVAR using variance under H1 with info_scale = 2")
  ) %>% 
  gt::tab_spanner(
    label = "Sample size wighting",
    columns = c(SS0, SS1, SS2)
  ) %>%
  cols_label(
    `INVAR using variance under H0 with info_scale = 0` = "info_scale = 0",
    `INVAR using variance under H0 with info_scale = 1` = "info_scale = 1",
    `INVAR using variance under H0 with info_scale = 2` = "info_scale = 2",
    `INVAR using variance under H1 with info_scale = 0` = "info_scale = 0",
    `INVAR using variance under H1 with info_scale = 1` = "info_scale = 1",
    `INVAR using variance under H1 with info_scale = 2` = "info_scale = 2",
    SS0 = "info_scale = 0",
    SS1 = "info_scale = 1",
    SS2 = "info_scale = 2"
  )
...
run_simulation <- function(N, weight_method, 
  ...
  for (simu in 1:n_sim) {
    ...
    x_e_S1 <- lapply(diff(c(0, ceiling(N_S1 / 2))), function(x) {
      rbinom(n = x, size = 1, prob = p_e$rate[1])
    })
    x_e_S2 <- lapply(diff(c(0, ceiling(N_S2 / 2))), function(x) {
      rbinom(n = x, size = 1, prob = p_e$rate[2])
    })
    x_e_S3 <- lapply(diff(c(0, ceiling(N_S3 / 2))), function(x) {
      rbinom(n = x, size = 1, prob = p_e$rate[3])
    })
    x_c_S1 <- lapply(diff(c(0, ceiling(N_S1 / 2))), function(x) {
      rbinom(n = x, size = 1, prob = p_c$rate[1])
    })
    x_c_S2 <- lapply(diff(c(0, ceiling(N_S2 / 2))), function(x) {
      rbinom(n = x, size = 1, prob = p_c$rate[2])
    })
    x_c_S3 <- lapply(diff(c(0, ceiling(N_S3 / 2))), function(x) {
      rbinom(n = x, size = 1, prob = p_c$rate[3])
    })
    ...
    if (integration == "gs_power_npe") {
      ...
      ans_new <- tibble::tibble(
        simu_ID = simu, method = weight_method,
    ...
    if (integration == "pmvtnorm") {
      ...
      p <- pmvnorm(
        ...
        corr = matrix(
          c(
            1, sqrt(IF_sim[1] / IF_sim[2]), sqrt(IF_sim[1] / IF_sim[3]),
...
ans_H0 <- do.call(
  ...
  lapply(
    list(
      list("ss", max(ans$SS0)),
      list("invar_H0", max(ans$`INVAR using variance under H0 with info_scale = 0`))
    ),
    function(x) {
      return(run_simulation(
        ...
        IF = IF, under_H0 = TRUE, integration = "pmvtnorm"
      ))
    }
  )
)
...
ans_H1 <- do.call(
  ...
  lapply(
    list(
      list("ss", ans$SS1),
      list("invar_H1", ans$`INVAR using variance under H1 with info_scale = 1`)
    ),
    function(x) {
      return(run_simulation(
        N = x[[2]],
...
ans %>%
  ...
  summarize(
    `simulated power - ss` = mean(ss),
    `simulated power - invar` = mean(invar)
  ) %>%
  gt::gt() %>%
  gt::tab_header(
    title = "Simulated power under different weightin methods",
