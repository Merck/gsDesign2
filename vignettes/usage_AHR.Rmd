...
fail_rate <- tibble(Stratum = "All",
                    ...
                    hr = c(.9, .75, .8, .6),
                    dropout_rate = .001)
AHR(enroll_rate = enroll_rate, fail_rate = fail_rate, total_duration = c(15, 30)) %>% gt::gt()
```


## Example 2: Stratified population
```{r}
enroll_rate <- tibble(Stratum = c(rep("Low", 2), rep("High", 3)),
                      duration = c(2, 10, 4, 4, 8),
                      rate = c(5, 10, 0, 3, 6))
fail_rate <- tibble(Stratum = c(rep("Low", 2), rep("High", 2)),
                    duration = 1,
                    fail_rate = c(.1, .2, .3, .4),
                    hr = c(.9, .75, .8, .6),
                    dropout_rate = .001)
...
```{r}
events_c <- expected_evnet(enroll_rate = enroll_c, fail_rate = fail_c, total_duration = td, simple = FALSE)
events_e <- expected_evnet(enroll_rate = enroll_e, fail_rate = fail_e, total_duration = td, simple = FALSE)
```
...
But notice that the above `enroll_rate` and `fail_rate` is not always the $H_1$, so we call it as `info`, rather than `info1`. 