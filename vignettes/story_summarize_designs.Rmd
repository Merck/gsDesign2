---
title: "Summarize group sequential designs in nice gt tables"
output:
...
vignette: |
  %\VignetteIndexEntry{Summarize group sequential designs in nice gt tables}
...
In the above default table summary table generated by `summary(x)`, the variables used to summarize each analysis includes `Analysis`, `Time`, `N`(sample size), `Events`, `AHR`, and `info_frac` (information fraction). 
