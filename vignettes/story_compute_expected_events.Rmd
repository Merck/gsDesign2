...
pdropRates <- dropout_rate * multiplier
...
```{r}
expected_event(
  enroll_rate = tibble(duration = c(1, 1), rate = c(3, 2)),
...
xx <- simtrial::simPWSurv(
  n = nsim,
  block = (rep("xx", 4)),
  enroll_rate = tibble(rate = c(3, 2) * nsim / 5, duration = c(1, 1)),
  fail_rate = tibble(
...
cutData <- function(x, cutDate) {
  x %>%
    ...
    mutate(
      tte = pmin(cte, cutDate) - enrollTime,
