...
- User experience: ensure a consistent API and syntax with minimal surprises
for using our software.
- Developer experience: minimize time and cost to make consistent interface
design decisions for developing our software.

Ultimately, the style guide should help us

- Support a healthy coexistence of user-facing, functional API, and low-level,
object-oriented API.
- Create balanced alignment with the style and best practices presented in other
...
Note that the package name and the product name can be stylized differently.
Example: tensorflow vs. TensorFlow.
...
This rule also applies to the S3 or R7 **method** names and argument names, as
they are inherently functions.
One should still follow the S3 convention to use dot (`.`) to connect the method
name and class name.
...
Object-oriented API should be selectively used for low-level abstractions and
limited cases in user-facing interactions when appropriate.

### Classes

Class names defined under an OOP system, for example, S3 or R7, should use
`UpperCamelCase`.

This rule intends to indicate the "function" is a class constructor, instead of
a (regular) exported or internal function and follows the conventions in R6.
...
This style makes it natural to consistently create chainable operations that
form a workflow under either a functional API or object-oriented API, with
minimal surprises.
...
This suggests that if the "data" argument requires multiple inputs, create
another function to bundle them as a single object and pass it to the function
of interest.

### Required and optional arguments

[Required arguments](https://design.tidyverse.org/def-required.html) should
**not** have a default value. In contrast, optional arguments should have a
default value.
...
The reasoning behind this: the strings are used together with function names and
argument names, using `snake_case` would create a visual consistency.

Whenever possible, set the default value of an enumerated type input in the
argument definition. See
[enumerate possible options](https://design.tidyverse.org/def-enum.html).

## Code formatting

R code should be formatted using [styler](https://styler.r-lib.org/) with the
default tidyverse style.

C/C++ code should be formatted using clang-format. This is doable via the
