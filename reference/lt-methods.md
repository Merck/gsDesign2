...
S3 methods for [`lt()`](https://rdrr.io/pkg/lt/man/lt.html) that convert
fixed or group sequential design summaries into formatted lt tables.

## Usage

``` r
# S3 method for class 'fixed_design_summary'
lt(data, title = NULL, footnote = NULL, ...)
...
- title:
  ...
  A string to specify the title of the table.
...
- footnote:
  ...
  `c("colname", "title", "subtitle", "analysis", "spanner")`. To disable
  footnotes, use `footnote = FALSE`.
...
- subtitle:
  ...
  A string to specify the subtitle of the table.

- colname_spanner:

  A string to specify the spanner of the table.

- colname_spannersub:

  A vector of strings to specify the spanner details of the table.
